...
export const NetworkSelectionDialog: React.FC<NetworkSelectionDialogProps> = ({
  ...
  const renderTerminalSelection = () => (
    <div className='text-center'>
      <Title level={3} className='mb-6'>
        <LocalizedText>
          {t('network.selectTerminalType', 'Select Terminal Type')}
        </LocalizedText>
      </Title>
          ...
          >
            <Card.Meta
              title={
                <div className='text-center'>
                  <LocalizedText>
                    {t('network.mainComputer', 'Main Computer')}
                  </LocalizedText>
                </div>
              }
              description={
                <div className='text-center'>
                  <Paragraph>
                    <LocalizedText>
                      {t(
                        'network.mainComputerDesc',
                        'This computer will host the database and serve other terminals'
                      )}
                    </LocalizedText>
                  </Paragraph>
                  <ul className='text-left text-sm'>
                    <li>
                      <LocalizedText>
                        {t('network.mainFeature1', 'Hosts the main database')}
                      </LocalizedText>
                    </li>
                    <li>
                      <LocalizedText>
                        {t('network.mainFeature2', 'Manages file sharing')}
                      </LocalizedText>
                    </li>
                    <li>
                      <LocalizedText>
                        {t('network.mainFeature3', 'Controls system settings')}
                      </LocalizedText>
                    </li>
                    <li>
                      <LocalizedText>
                        {t('network.mainFeature4', 'Generates reports')}
                      </LocalizedText>
                    </li>
                  </ul>
                </div>
          ...
          >
            <Card.Meta
              title={
                <div className='text-center'>
                  <LocalizedText>
                    {t('network.clientComputer', 'Client Computer')}
                  </LocalizedText>
                </div>
              }
              description={
                <div className='text-center'>
                  <Paragraph>
                    <LocalizedText>
                      {t(
                        'network.clientComputerDesc',
                        'This computer will connect to the main computer for data access'
                      )}
                    </LocalizedText>
                  </Paragraph>
                  <ul className='text-left text-sm'>
      ...
      <Button type='link' onClick={handleSkipToOffline}>
        <LocalizedText>
          {t('network.skipToOffline', 'Skip and use offline mode')}
        </LocalizedText>
      </Button>
    </div>
  );
    ...
    <div>
      ...
      >
        <Form.Item
          label={
            <LocalizedText>
              {t('network.terminalName', 'Terminal Name')}
            </LocalizedText>
          }
          name='terminalName'
          rules={[
            { required: true, message: 'Please enter a terminal name' },
            {
              validator: (_, value) => {
                ...
                return validation.valid
                  ? Promise.resolve()
                  : Promise.reject(validation.message);
              },
            },
          ]}
        >
          <Input
            placeholder={t(
              'network.terminalNamePlaceholder',
        ...
        {config.terminalType === 'client' && (
          <Form.Item
            label={
              <LocalizedText>
                {t('network.mainComputerPath', 'Main Computer Path')}
              </LocalizedText>
            }
            ...
            rules={[
              {
                required: true,
                message: 'Please enter the main computer path',
              },
              {
                validator: (_, value) => {
          ...
          <Col span={16}>
            <div>
              ...
              <Text type='secondary'>
                {config.terminalType === 'main' ? (
                  <LocalizedText>
                    {t('network.mainComputerSetup', 'Main Computer Setup')}
                  </LocalizedText>
                ) : (
                  <LocalizedText>
                    {t('network.connectingTo', 'Connecting to')}:{' '}
  ...
  const renderCompletion = () => (
    <div className='text-center'>
      ...
      <Title level={3} className='mb-4'>
        <LocalizedText>
          {t('network.setupComplete', 'Network Setup Complete!')}
        </LocalizedText>
      </Title>
      ...
      <Paragraph>
        <LocalizedText>
          {config.terminalType === 'main'
            ? t(
                'network.mainSetupSuccess',
                'Your main computer has been configured successfully. Other terminals can now connect to this computer.'
              )
            : t(
                'network.clientSetupSuccess',
                'Your client computer has been connected successfully. You can now use the POS system.'
              )}
        </LocalizedText>
      </Paragraph>
      ...
      <Card className='mb-4'>
        <Row gutter={16}>
          <Col span={12}>
            <Text strong>
              <LocalizedText>
                {t('network.terminalType', 'Terminal Type')}
              </LocalizedText>
              :
            </Text>
            <br />
        ...
        {config.mainComputerPath && (
          <Row gutter={16} className='mt-2'>
            <Col span={24}>
              <Text strong>
                <LocalizedText>
                  {t('network.connectedTo', 'Connected To')}
                </LocalizedText>
                :
              </Text>
              <br />
      ...
      <Button type='primary' size='large' onClick={handleComplete}>
        <LocalizedText>
          {t('network.startUsingPOS', 'Start Using POS System')}
        </LocalizedText>
      </Button>
    </div>
  );
  ...
  const steps = [
    {
      title: (
        <LocalizedText>{t('network.selectType', 'Select Type')}</LocalizedText>
      ),
      icon: <SettingOutlined />,
    },
    {
      title: (
        <LocalizedText>{t('network.configure', 'Configure')}</LocalizedText>
      ),
    ...
    <Modal
      title={
        <div>
          ...
          <LocalizedText>
            {t('network.networkSetup', 'Network Setup')}
          </LocalizedText>
        </div>
      }
      open={visible}
